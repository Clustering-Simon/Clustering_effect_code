...
transition_space* init_transition() {
  ...
  for (int beta=0; beta < 1<<PRECISION; beta++) {
    // Compute vector space
    uint32_t B = ROT(beta,-c);
    uint32_t X[32];
    for (int i=0; i<32; i++) {
      X[i] = ROT((beta&ROT((1<<i),b-a)) ^ ROT(beta&(1<<i),-b+a),-a);
    }
    // Gaussian reduce
    int p = 31;
    for (int i=0; i<32; i++) {
...
int main() {
  ...
  for (int i=1; i<ROUNDS; i++) {
    ...
    {
      // Print distribution
      for (int i = 0; i < 1<<PRECISION; i += 1<<(PRECISION-6)) {
	printf ("[%6i]: ", i);
	for (int j = 0; j < 1<<PRECISION; j += 1<<(PRECISION-6)) {
	  double m = log2(d[i][j]);
	  printf (" %6.2f", m);
	}
	printf ("\n");
